...
#[public]
impl Contract {
    /// Mint rendered image using input parameters as DNA
    /// Re-computes pixel computation internally
    pub fn mint_rendered_image(
        &mut self,
        sphere_r: u8,
        sphere_g: u8,
        sphere_b: u8,
        ...
        // Simple gradient rendering inline (DNA-based)
        ...
        for j in 0..HEIGHT {
            for i in 0..WIDTH {
                ...
                let final_r = if dist_x < 8 && dist_y < 8 { sphere_r } else { r };
                let final_g = if dist_x < 8 && dist_y < 8 { sphere_g } else { g };
                let final_b = if dist_x < 8 && dist_y < 8 { sphere_b } else { b };
        ...
        // Return token ID 1 (minting successful)
        1u64
    }
    ...
    pub fn render_scene(
        ...
        // Simple gradient with sphere - efficient and proven
        let mut pixels = Vec::with_capacity(WIDTH * HEIGHT * 3);
